...
public class OdontologiaController {
    ...
    public ResponseEntity<byte[]> generarPdf(Long idOdon) throws JRException, FileNotFoundException {
        JRBeanCollectionDataSource beanCollectionDataSource = new JRBeanCollectionDataSource(service.generarPdf(idOdon));
        if (beanCollectionDataSource.getData().size() > 0) {
            JasperReport compileReport = JasperCompileManager.compileReport(new FileInputStream("src/main/resources/Reports/Odontologia.jrxml"));
            HashMap<String, Object> map = new HashMap<>();
            JasperPrint report = JasperFillManager.fillReport(compileReport, map, beanCollectionDataSource);
            OdontologiaDto odontologiaD = service.generarPdf(idOdon).get(0);
            int idOd = odontologiaD.getIdOdonto();
            byte[] data = JasperExportManager.exportReportToPdf(report);
            ContentDisposition contentDisposition = ContentDisposition.builder("attachment")
                    .filename("Ficha Odontologia" + idOd + "_" + generarCodigoAleatorio() + ".pdf")
                    .build();
